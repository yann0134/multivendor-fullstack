          ...
          <v-card class="auth-card agri-fade-in" elevation="24">
            ...
            <v-card-text class="pa-8">
              <v-form @submit.prevent="handleSubmit" class="auth-form">
                ...
                <v-btn
                  ...
                  block
                  :loading="loading"
                  :disabled="!form.email || (!otpSent && !form.role)"
                >
                  <v-icon left>{{ otpSent ? 'mdi-login' : 'mdi-send' }}</v-icon>
                  {{ otpSent ? 'Se connecter' : 'Envoyer le code' }}
                </v-btn>
            ...
            <v-card-actions class="pa-6 pt-0">
              ...
              <div class="text-center w-100">
                <span class="text-body-2 text-grey-600 mr-2">Pas encore de compte ?</span>
...
// Configuration des rôles avec icônes selon le workflow AgriMarket
const roles = [
  { value: 'ROLE_CUSTOMER', label: 'Client', icon: 'mdi-account', description: 'Achetez des produits frais' },
  { value: 'ROLE_SUPPLIER', label: 'Fournisseur', icon: 'mdi-sprout', description: 'Fournissez vos produits agricoles' },
  { value: 'ROLE_ADMIN', label: 'Administrateur', icon: 'mdi-shield-check', description: 'Validez et gérez la plateforme' },
  { value: 'ROLE_WAREHOUSE', label: 'Entrepôt', icon: 'mdi-warehouse', description: 'Gérez le stock central' },
  { value: 'ROLE_DELIVERY', label: 'Agent/Livreur', icon: 'mdi-truck', description: 'Récupérez et livrez les produits' }
]
