    ...
    <v-row>
      <v-col cols="12">
        ...
        <v-card class="pa-6">
          ...
          <v-form @submit.prevent="updateRecipe" ref="form" v-if="recipe">
            <v-row>
...
const pricePerServing = computed(() => {
  return recipe.value && recipe.value.servings > 0 ? Math.round(recipe.value.totalPrice / recipe.value.servings) : 0
})
...
const calculateTotalPrice = () => {
  ...
  recipe.value.ingredients.forEach(ingredient => {
    if (ingredient.quantity) {
      ...
      if (ingredient.product && ingredient.product.sellingPrice) {
        price = ingredient.product.sellingPrice
      }
...
const updateRecipe = async () => {
  ...
  try {
    ...
    alert('Erreur lors de la mise à jour de la recette: ' + (error.response?.data?.message || error.message))
  } finally {
    loading.value = false
  }
}
